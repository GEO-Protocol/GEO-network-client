#include "MaxDemianSecondMigration.h"

MaxDemianSecondMigration::MaxDemianSecondMigration(
    sqlite3 *dbConnection,
    Logger &logger):
    mDataBase(dbConnection),
    mLog(logger)
{
    mNeighborUUIDFirst = NodeUUID("5848f113-dcb6-4d1d-86fc-378fbf1778d4");
    mNeighborUUIDSecond = NodeUUID("be52cfae-9457-4a04-b62c-478416759c51");

    mOperationAmountFirst = TrustLineAmount(3917);
    mOperationAmountSecond = TrustLineAmount(7845);

    mBalanceAfterOperation = mOperationAmountSecond - mOperationAmountFirst;

    mOperationTimeStampOnCoordinatorNode = 44791775256137;
}

void MaxDemianSecondMigration::applyTrustLineMigration(
    IOTransaction::Shared ioTransaction)
{
    ...
    if (trustLine_second and trustLine_first) {
        ...
        auto new_balance = trustLine_first->balance() - mOperationAmountFirst;
        trustLine_first->setBalance(new_balance);
        ioTransaction->trustLinesHandler()->saveTrustLine(trustLine_first);

        new_balance = trustLine_second->balance() + mOperationAmountSecond;
...
void MaxDemianSecondMigration::applyTrustLineHistoryMigration(IOTransaction::Shared ioTransaction)
{

    auto previousPaymentRecord = getPreviousPaymentRecord(ioTransaction);

    auto balanceAfterOperation = previousPaymentRecord->balanceAfterOperation() + mBalanceAfterOperation;
...
void MaxDemianSecondMigration::apply(IOTransaction::Shared ioTransaction) {

    applyTrustLineMigration(ioTransaction);
    applyTrustLineHistoryMigration(ioTransaction);
}

std::shared_ptr <TrustLine> MaxDemianSecondMigration::getOutwornTrustLine(
    IOTransaction::Shared ioTransaction,
    NodeUUID &nodeUUID) {
    sqlite3_stmt *stmt;
    ...
    if (rc != SQLITE_OK) {
        throw IOError(
            "MaxDemianSecondMigration::getOutwornTrustLine: "
...
PaymentRecord::Shared MaxDemianSecondMigration::getPreviousPaymentRecord(
    IOTransaction::Shared ioTransaction)
{
    sqlite3_stmt *stmt;
    ...
    if (rc != SQLITE_OK) {
        throw IOError(
            "MaxDemianSecondMigration::getPreviousPaymentRecord: "
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW ) {
        ...
        throw NotFoundError("MaxDemianSecondMigration::getPreviousPaymentRecord:  "
...
PaymentRecord::Shared MaxDemianSecondMigration::deserializePaymentRecord(
    sqlite3_stmt *stmt)
{
...
void MaxDemianSecondMigration::updatePaymentsRecords(
    IOTransaction::Shared ioTransaction,
    vector<PaymentRecord::Shared> paymentsRecordsToUpdate)
{
    for(auto record: paymentsRecordsToUpdate){
        ...
        if (rc != SQLITE_OK) {
            throw IOError(
                "MaxDemianSecondMigration::updatePaymentsRecords: "
...
vector<PaymentRecord::Shared> MaxDemianSecondMigration::getPaymentRecordsForUpdate(
    IOTransaction::Shared ioTransaction,
    int64_t since_what_timestamp)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW ) {
        ...
        TrustLineBalance balanceAfterOperation = bytesToTrustLineBalance(
            balanceBytes);
        // Update Balance
        balanceAfterOperation = balanceAfterOperation + mBalanceAfterOperation;
...
pair<BytesShared, size_t> MaxDemianSecondMigration::serializedPaymentRecordBody(
