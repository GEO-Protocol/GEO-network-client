        ...
        identifier())
{
    ...
    auto flags_add = [&](auto &ctx) { mFlags = _attr(ctx); };
    auto mforbiddenamount_add = [&](auto &ctx)
    {

        forbiddenamount += _attr(ctx);
        flag++;
        if(flag>39) {throw ValueError("Amount is too big");}
        else if (flag == 1 && _attr(ctx) <= 0) {throw ValueError("Amount can't be zero or low");}
